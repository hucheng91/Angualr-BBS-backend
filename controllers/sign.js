...
exports.signup = function (req, res, next) {
  ...
  ep.on('prop_err', function (msg) {
    res.status(403);
    res.send({error: msg, loginname: loginname, email: email});
  });
...
exports.login = function (req, res, next) {
  ...
  getUser(loginname, function (err, user) {
    ...
    if (!user) {
      ...
      Message.getMessagesCount(user._id,function (count) {
          user.messages_count = !count ? 0 : count;
      });
    ...
    tools.bcompare(pass, passhash, ep.done(function (bool) {
      ...
      if (!user.active) {
        ...
        res.status(403);
         res.send({ error: '此帐号还没有被激活，激活链接已发送到 ' + user.email + ' 邮箱，请查收。'});
      }
