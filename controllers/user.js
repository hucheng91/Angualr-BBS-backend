...
exports.checkLogin = function (req,res,next) {
    ...
    var isLogin = !currentUser ? false : true;

    currentUser = _.pick(currentUser, ['_id','accessToken','active','avatar',,'collect_tag_count','collect_topic_count','follower_count',
        'following_count','is_block','loginname','name','pass','receive_at_mail','receive_reply_mail','reply_count','score',
        'topic_count','messages_count']);
    //currentUser.messages_count = _.pick(currentUser, ['messages_count']);
    res.send({isLogin: isLogin, user: currentUser});
}
exports.index = function (req, res, next) {
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    if (!user) {
      res.status(402).send('the user not exit');
      return;
    }
  ...
  //  var opt = {limit: 5, sort: '-create_at'};
    ...
    Reply.getRepliesByAuthorId(user._id, {sort: '-create_at'},
...
exports.setting = function (req, res, next) {
  ...
  if (action === 'change_setting') {
    ...
    User.getUserById(req.session.user._id, ep.done(function (user) {
      ...
      user.save(function (err) {
        ...
        return res.send({success:true,message:"修改成功！"});
      });
    }));
  }
  if (action === 'change_password') {
    ...
    if (!old_pass || !new_pass) {
      return res.status(403).send({success: false,message:'旧密码或新密码不得为空'});
    }

    User.getUserById(req.session.user._id, ep.done(function (user) {
      tools.bcompare(old_pass, user.pass, ep.done(function (bool) {
        if (!bool) {
          return res.status(403).send({success: false,message:"当前密码不正确"});
        }
        ...
        tools.bhash(new_pass, ep.done(function (passhash) {
          ...
          user.save(function (err) {
            ...
            return res.send({success: true,message:"密码已被修改"});
