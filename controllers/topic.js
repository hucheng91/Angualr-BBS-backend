...
exports.index = function (req, res, next) {
  ...
  Topic.getFullTopic(topic_id, ep.done(function (message, topic, author, replies) {
    ...
    topic = topic.toObject();  // 序列化一把
    var md = new MarkdownIt();
    topic.content = md.render(topic.content);
    topic.create_at = tools.formatDate(topic.create_at,true);
    topic.update_at = tools.formatDate(topic.update_at,true);
    topic.author  = author;
    topic.replies = replies.map(function (reply) {
        reply.noMarkdownContent = reply.content;
        reply.content = md.render(reply.content);
        reply.author = _.pick(reply.author, ['_id','loginname', 'avatar_url']);
        reply =  _.pick(reply, ['id','author','noMarkdownContent', 'content', 'ups', 'create_at', 'reply_id']);
        reply.reply_id = reply.reply_id || null;
        reply.create_at = tools.formatDate(reply.create_at,true);
...
exports.put = function (req, res, next) {
  ...
  if (editError) {
    return  res.status(422).send(editError);
  }
...
exports.showEdit = function (req, res, next) {
  ...
  Topic.getTopicById(topic_id, function (err, topic, tags) {
    if (!topic) {
        return  res.status(422).send('此话题不存在或已被删除。');
    }
